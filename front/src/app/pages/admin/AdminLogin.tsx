...
export function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await adminLogin({ PhoneNumber: phoneNumber, Password: password });
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-border p-8">
          {IS_MOCK && (
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 text-xs rounded-lg px-3 py-2 mb-6 text-center">
              حالت تست — شماره: <span dir="ltr" className="font-mono">09000000000</span> / رمز: <span dir="ltr" className="font-mono">Test@1234</span>
            </div>
          )}
