...
import { Star, MapPin, Clock, Phone, ChevronLeft, Loader2, AlertCircle } from 'lucide-react';
import { getRestaurantById } from '../services/restaurant.service';
import { formatJalaliDate } from '../utils/jalali';
import type { Restaurant } from '../types';
import { Heart } from 'lucide-react';   // به import‌های lucide اضافه کن
import { toast } from 'sonner';
import { isFavorited, toggleFavorite } from '../services/favorite.service';
...
export function RestaurantDetails() {
  ...
  const [isFav, setIsFav] = useState(() => isFavorited(id ?? ''));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate(-1)} className="text-primary hover:underline">بازگشت</button>
        </div>
      </div>
    );
  }
                ...
                {(restaurant.rating ?? 0) > 0 && (
                  <>
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium">{formatRating(restaurant.rating ?? 0)}</span>
                ...
                {activeTab === 'menu' && (
                  <div className="space-y-4">
                    {restaurant.MenuItems.length === 0 ? (
                      <p className="text-muted-foreground text-center py-8">منویی ثبت نشده است</p>
                    ) : (
                  ...
                  <div className="space-y-6">
                    {(restaurant.rating ?? 0) > 0 && (
                      <div className="flex items-center gap-6 pb-6 border-b border-border">
                        <div className="text-center">
                          <div className="text-4xl font-bold text-primary mb-1">{formatRating(restaurant.rating ?? 0)}</div>
                          ...
                          {[5, 4, 3, 2, 1].map((star) => (
                            <div key={star} className="flex items-center gap-2">
                              ...
                              <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                                <div className="h-full bg-yellow-400 rounded-full" style={{ width: `${star * 18}%` }} />
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border border-border sticky top-20">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {/* نمایش شمسی */}
                  {selectedDate && (
                    <p className="text-xs text-primary mt-1 text-left" dir="rtl">
                ...
                {!canReserve && (
                  <p className="text-xs text-muted-foreground text-center">
                    برای ادامه تاریخ و ساعت را انتخاب کنید
                  </p>
                )}
                ...
                <button
                  ...
                  disabled={!canReserve}
                  className="block w-full px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors text-center disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ادامه رزرو
                </button>
